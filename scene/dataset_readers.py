...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    lowfps_stride = int(os.environ.get("LOWFPS_STRIDE", "0"))
    if eval and lowfps_stride > 0:
        # 低帧率协议：train 每 stride 帧抽稀（20fps/4→5fps），test 取每个 gap 正中（novel time）。
        # 事件监督仍用 move_cam_event 的密集位姿 → 稀疏 RGB + 密集事件。
        train_cam_infos = [c for i, c in enumerate(cam_infos) if i % lowfps_stride == 0]
        test_cam_infos = [c for i, c in enumerate(cam_infos) if i % lowfps_stride == lowfps_stride // 2]
    elif eval:
